...
const emptyBrand = { name: '', slug: '', logo: '', description: '', order: 0 }

export default function ManageBrands() {
  const { brands, createBrand, updateBrand, deleteBrand, fetchBrands } = useData()
  ...
  const openAdd = () => { setForm({ ...emptyBrand }); setEditing(null); setShowForm(true) }
  const openEdit = (brand) => { setForm({ ...brand }); setEditing(brand._id); setShowForm(true) }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) { await updateBrand(editing, data); toast.success('Brand updated!') }
      else { await createBrand(data); toast.success('Brand created!') }
      setShowForm(false); setEditing(null); setForm({ ...emptyBrand })
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteBrand(id); toast.success('Brand deleted!') }
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)]">
      <header className="bg-[var(--color-paper)] border-b border-[var(--color-line)] sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg font-bold text-[var(--color-ink)]" style={{ fontFamily: 'var(--font-italiana)' }}>Manage Brands</h1>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div className="bg-[var(--color-paper)] rounded-2xl p-6 w-full max-w-lg border border-[var(--color-line)] shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-bold text-[var(--color-ink)]" style={{ fontFamily: 'var(--font-italiana)' }}>{editing ? 'Edit Brand' : 'Add Brand'}</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-[var(--color-muted)] mb-1.5 uppercase tracking-wider">Logo Image</label>
                  ...
                  {form.logo && <div className="mt-3 w-full h-40 rounded-xl overflow-hidden border border-[var(--color-line)] softyy-media-frame p-3"><img src={form.logo} alt="Preview" className="softyy-media-contain" /></div>}
                </div>
                <div>
        ...
        <div className="space-y-4">
          {brands.length === 0 && <p className="text-center py-16 text-[var(--color-muted)]">No brands yet.</p>}
          {brands.map((brand) => (
            <div key={brand._id} className="bg-[var(--color-paper)] rounded-2xl p-5 border border-[var(--color-line)] flex gap-5 items-center hover:shadow-lg transition-shadow">
              <div className="w-20 h-20 rounded-xl overflow-hidden border border-[var(--color-line)] shrink-0 softyy-media-frame p-2">
                {brand.logo ? <img src={brand.logo} alt={brand.name} className="softyy-media-contain" /> : <div className="w-full h-full flex items-center justify-center text-[var(--color-muted)] text-xs">No img</div>}
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-[var(--color-ink)] text-sm">{brand.name}</h3>
                <p className="text-xs text-[var(--color-muted)] truncate mt-0.5">{brand.description}</p>
                <span className="text-[0.65rem] text-[var(--color-muted)]">Slug: {brand.slug}</span>
              </div>
              <div className="flex gap-2 shrink-0">
                <button onClick={() => openEdit(brand)} className="w-9 h-9 rounded-xl flex items-center justify-center text-[var(--color-wine)] hover:bg-[var(--color-wine)]/10 transition-colors"><Edit3 size={16} /></button>
                <button onClick={() => handleDelete(brand._id)} className="w-9 h-9 rounded-xl flex items-center justify-center text-red-400 hover:bg-red-50 transition-colors"><Trash2 size={16} /></button>
              </div>
            </div>
          ))}
        </div>
